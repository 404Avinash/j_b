...
class IntelMLTrainer:
    ...
    def load_existing_intel(self, path: str = None) -> pd.DataFrame:
        ...
        if os.path.exists(path):
            ...
            # Check for parts
            import glob
            base_name = os.path.basename(path).replace('.csv', '_part*.csv')
            part_pattern = os.path.join(os.path.dirname(path), base_name)
            parts = sorted(glob.glob(part_pattern))
            
            if parts:
                print(f"[ML] Loading from {len(parts)} split files...")
                dfs = [pd.read_csv(p) for p in parts]
                self.intel_df = pd.concat(dfs, ignore_index=True)
                # Convert dates
                if 'Timestamp' in self.intel_df.columns:
                    self.intel_df['Timestamp'] = pd.to_datetime(self.intel_df['Timestamp'])
                if 'Date' in self.intel_df.columns:
                    self.intel_df['Date'] = pd.to_datetime(self.intel_df['Date'])
            else:
                raise FileNotFoundError(f"No data found at {path} or split parts")
